	...
	public ArrayList<Order> getOrders(String phonenum) throws Exception {
		...
		if (responseCode == 200) {
			JSONArray ordersJson = client.convertStringToJSONArray(client
					.getResponse());
			// if there are no orders found (either no such customer id or just no orders
			...
			for (int i = 0; i < ordersJson.length(); i++)
			{
				...
				java.util.Date date = sdf.parse(orderJson.getString("time_ordered")); 
				java.sql.Timestamp timest = new java.sql.Timestamp(date.getTime()); 
				order.setTimeOrdered(timest);
				
				date = sdf.parse(orderJson.getString("requested_time_to_deliver"));
				...
				date = sdf.parse( orderJson.getString("replied_time_to_deliver"));
				timest = new Timestamp(date.getTime());
				...
				order.setOrderReplyMessage(orderJson.getString("replied_message"));
			...
			Log.v(TAG, "order1.car1 " + orders.get(0).getCars().get(0).getType());
